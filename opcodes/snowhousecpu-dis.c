...
snowhousecpu_dasm_info_do_disassemble (snowhousecpu_dasm_info_t *self)
{
  ...
  const snowhousecpu_temp_t temp_simm24 = (
    snowhousecpu_get_insn_field (SNOWHOUSECPU_SIMM24_MASK, SNOWHOUSECPU_SIMM24_BITPOS, self->iword)
  );
  snowhousecpu_temp_t temp_simm32 = snowhousecpu_sign_extend (
    temp_simm16, SNOWHOUSECPU_IMM16_BITSIZE
  );
  snowhousecpu_temp_t temp_simm24_to_32 = snowhousecpu_sign_extend (
    temp_simm24, SNOWHOUSECPU_SIMM24_BITSIZE
  );
  self->uimm = temp_simm16;
  self->simm = temp_simm32;
  self->simm24 = temp_simm24_to_32;
...
#define do_snprintf_insn_snowhousecpu_main_worker(...) \
  do \
  { \
    int start = (int)0; \
    \
    if (args->show_enc_instr) \
    { \
      start = snprintf (temp_buf, temp_buf_lim, "enc:%lx", args->iword); \
    } \
    \
    if (start >= 0 && (size_t)start < temp_buf_lim) \
    { \
      snprintf (temp_buf + start, temp_buf_lim - start, __VA_ARGS__); \
    } \
  } while (0)
...
)
{
  switch (args->opc_info->oparg)
  {
    ...
    case SNOWHOUSECPU_OA_NONE:
    {
      if (args->length != 4)
      {
        do_snprintf_insn_snowhousecpu_main_worker ("%s%x%s",
          "bad (op 0x",
          (unsigned) args->op,
      ...
      {
	do_snprintf_insn_snowhousecpu_main_worker ("%s",
	  args->opc_info->name);
      }
    }
      break;
    ...
    case SNOWHOUSECPU_OA_RA_RB:
    {
      ...
      {
	do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %s",
	  args->opc_info->name,
    ...
    case SNOWHOUSECPU_OA_RA_RB_RC:
    {
      ...
      {
	do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %s, %s",
	  args->opc_info->name,
	  gprs[args->ra_idx].name,
	  gprs[args->rb_idx].name,
    ...
    {
      do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %s, %i%s",
	args->opc_info->name,
	gprs[args->ra_idx].name,
	gprs[args->rb_idx].name,
	(signed) args->simm,
    ...
    case SNOWHOUSECPU_OA_RA_RB_SHIFT_U5:
    {
      ...
      {
	do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %s, %u",
	  args->opc_info->name,
	  gprs[args->ra_idx].name,
	  gprs[args->rb_idx].name,
	  (unsigned) args->uimm);
    ...
    //{
    //  do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %i%s",
    //    args->opc_info->name,
    //    gprs[args->ra_idx].name,
    //    (signed) args->simm,
    //    do_snprintf_insn_snowhousecpu_maybe_pre (args));
    ...
    {
      do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %s, %i /* dst_pc=0x%x */%s",
	args->opc_info->name,
	gprs[args->ra_idx].name,
	gprs[args->rb_idx].name,
	(signed) args->simm,
	((unsigned) args->simm) + ((unsigned) args->curr_pc) + 4u,
	do_snprintf_insn_snowhousecpu_maybe_pre (args));
    }
      break;
    case SNOWHOUSECPU_OA_RA_PCREL_S24:
    {
      do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %i /* dst_pc=0x%x */%s",
	args->opc_info->name,
	gprs[args->ra_idx].name,
	(signed) args->simm24,
	((unsigned) args->simm24) + ((unsigned) args->curr_pc) + 4,
	do_snprintf_insn_snowhousecpu_maybe_pre (args));
    }
      break;
    case SNOWHOUSECPU_OA_PCREL_S24_IMPLICIT_LR:
    {
      do_snprintf_insn_snowhousecpu_main_worker ("%s\t%i /* dst_pc=0x%x */%s",
	args->opc_info->name,
	//gprs[args->ra_idx].name,
	//gprs[args->rb_idx].name,
	(signed) args->simm24,
	...
	gprs[args->ra_idx].name,
	(signed) args->simm,
	((unsigned) args->simm) + ((unsigned) args->curr_pc) + 4,
	do_snprintf_insn_snowhousecpu_maybe_pre (args));
    }
      break;
    ...
    case SNOWHOUSECPU_OA_IRA:
    {
      ...
      {
	do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s",
	  args->opc_info->name,
	  sprs[SNOWHOUSECPU_SPR_ENUM_IRA].name);
      }
    }
    ...
    //  else
    //  {
    //    do_snprintf_insn_snowhousecpu_main_worker ("%s\t%s, %s, %s, %s",
    //      args->opc_info->name,
    //      sprs[SNOWHOUSECPU_SPR_ENUM_LO].name,
    //      gprs[args->ra_idx].name,
    //      gprs[args->rb_idx].name,
    ...
    default:
      //if (
      //  args->show_enc_instr
      //  && args->opc_info->oparg == SNOWHOUSECPU_OA_PRE_S16
      //)
      //{
      //  do_snprintf_insn_snowhousecpu_main_worker (
      //    "pre #%lx",
      //  );
      //}
      //else
      {
	//do_snprintf_insn_snowhousecpu_main_worker ("bad (grp 0x%x; oparg 0x%x; opcode 0x%x; name %s)",
	//  (unsigned) args->grp, (unsigned) args->opc_info->oparg,
	//  (unsigned) args->opc_info->opcode,
	//  args->opc_info->names[args->fw]);

	do_snprintf_insn_snowhousecpu_main_worker ("%s%x%s%s%s",
	  "bad (op 0x",
	  (unsigned) args->op,
	  "; ",
	  args->opc_info->name,
...
#undef do_snprintf_insn_snowhousecpu_main_worker
...
)
{
  ...
  snowhousecpu_dasm_info_t dasm_info;
  snowhousecpu_dasm_info_ctor (&dasm_info, rd32_func, show_enc_instr);
  if (pc)
  {
    dasm_info.curr_pc = *pc;
  }
...
print_insn_snowhousecpu (bfd_vma addr, struct disassemble_info *info)
{
  ...
  snowhousecpu_dasm_info_t dasm_info;
  snowhousecpu_dasm_info_ctor (&dasm_info, &print_insn_snowhousecpu_rd32, false);
  dasm_info.curr_pc = loc_addr;
  snowhousecpu_dasm_info_do_disassemble (&dasm_info);
