...
static reloc_howto_type snowhousecpu_elf_howto_table [] =
{
  ...
  HOWTO (R_SNOWHOUSECPU_EH_32_PCREL,     /* type */
      ...
      0,                            /* src_mask */
      MINUS_ONE,                    /* dst_mask */
      false /* should this be true? */), /* pcrel_offset */
  /* -------- */
};
                                  ...
                                  bfd_vma relocation, bfd_vma addend)
{
  ...
  else // if (howto->pc_relative)
  {
    ...
    //printf ("snowhousecpu_elf_do_non_sub_imm_reloc: pc_relative 0: %i %i\n",
    //  (signed) relocation, (signed) address);
    if (howto->pcrel_offset)
    {
      relocation -= address;
    }
    //printf ("snowhousecpu_elf_do_non_sub_imm_reloc: pc_relative 1: %i\n",
    //  (signed) relocation);
    //relocation += 4;

    //printf ("snowhousecpu_elf_do_non_sub_imm_reloc: pc_relative 2: %i\n",
    //  (signed) relocation);
    BFD_ASSERT (
  ...
  for (rel=relocs; rel<relend; ++rel)
  {
    ...
    if (howto)
    {
      switch (howto->type)
      {
        ...
        {
          //if (howto->type == R_SNOWHOUSECPU_32)
          //{
          //  fprintf (
          //    stderr,
